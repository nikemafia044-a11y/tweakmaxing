...
| 11 | GUI a partir do `TweakMaxing.ps1` compilado | `.\Compile.ps1` e depois abrir `.\TweakMaxing.ps1` (elevado, sem `-TestMode`), numa máquina **sem** o repositório | Janela abre; `%LOCALAPPDATA%\TweakMaxing\ui\<versão>\` e `...\lib\<versão>\` criados com os arquivos da interface e as 3 DLLs; as cinco abas carregam catálogo; MicroWin gera o `autounattend.xml` a partir do modelo embutido | |
| 12 | Release publicada de verdade | `irm https://github.com/<repo>/releases/download/v<versão>/TweakMaxing.ps1 \| iex` numa máquina limpa (sem o repositório clonado) | GUI abre normalmente; comparando com o caminho **Verificado** (`Invoke-WebRequest` + `Get-FileHash` contra `SHA256SUMS.txt` publicado na mesma release), o hash bate | |
